...
q_values_file = open('q_value_distributions_bonus.txt', 'w')
...
network_name = "q_network_bonus.pth"
current_state = None
action_idx = None
episode_score = 0
rewards = []
...
level_fixed_bonus = False
fixed_bonus = False
only_levelled_fixed = False
double_levelled_while_fixed_bonus = False

def get_reward(state, next_state):
    global fixed_bonus, level_fixed_bonus, only_levelled_fixed, double_levelled_while_fixed_bonus
    ...
    # Bonus system
    if not fixed and next_fixed and not fixed_bonus:
        fixed_bonus = True
        reward += 5
    if not level_fixed and next_level_fixed and not level_fixed_bonus:
        level_fixed_bonus = True
        reward += 10
    if level_fixed and (not next_a_fixed or not next_b_fixed) and next_level_fixed and not only_levelled_fixed:
        only_levelled_fixed = True
        reward += 5
    if level_fixed and next_a_levelled and next_b_levelled and not double_levelled_while_fixed_bonus:
        double_levelled_while_fixed_bonus = True
        reward += 10
...
def reset_env():
    global data, episode_counter, episode_score, rewards, robot, epsilon, epsilon_decay, fixed_bonus, level_fixed_bonus, only_levelled_fixed, double_levelled_while_fixed_bonus
    episode_counter += 1
    data = mj.MjData(model)
    robot.update_data(data)
    episode_score = 0
