...
state_space = list(itertools.product(
    ...
    [False, True],    # b_fixed
    [False, True],    # lifted
    range(-100, 100),   # a_range
    range(-100, 100),   # b_range
    [False, True],    # a_correct_height
    [False, True]     # b_correct_height
))
...
def controller(model, data):
    ...
    if not done and not robot.locked:
        encoded_state = [float(s) for s in state[:3]] + [state[3] /
                                                         100.0, state[4] / 100.0] + [float(s) for s in state[5:]]
        ...
        next_encoded_state = [float(s) for s in next_state[:3]] + [
            next_state[3] / 100.0, next_state[4] / 100.0] + [float(s) for s in next_state[5:]]
