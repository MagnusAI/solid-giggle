...
highscore = float('-inf')
rewards = []
network_name = "q_network_v36_norev.pth"
...
training_time_limit = 60 * 60 * 1  # seconds
...
def stage_reward(next_state):
    global success
    next_a_fixed, next_b_fixed, next_arm_angle, next_a_distance, next_b_distance, next_a_leveled, next_b_leveled = next_state

    next_fixed = next_a_fixed or next_b_fixed
    next_levelled = next_a_leveled or next_b_leveled
    ...
    if (success < 100):
        ...
        if(next_fixed and next_levelled):
            return 100
    elif (success < 300):
        if(next_fixed and next_levelled and next_a_distance < 10 and next_b_distance < 10):
            return 100
    elif (success < 400):
        if(next_fixed and next_level_fixed):
            return 100
    return 0